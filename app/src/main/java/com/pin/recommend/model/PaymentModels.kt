...
class PaymentBetweenCharacterDatesModel(private val paymentDao: PaymentDao, private val characterDao: RecommendCharacterDao) {
    ...
    val groupingByDate = characterId.switchMap { characterId ->
        section.switchMap { dates ->
            paymentDao.findByTrackedCharacterIdPaymentAndTagInDate(characterId ?: -1, dates.startDate, dates.endDate).map {
                it.groupBy {
    ...
    companion  object class Section(
      val startDate: Date,
      val endDate: Date
    )
...
class CharacterMonthlyPaymentModel(private val paymentDao: PaymentDao, private val characterDao: RecommendCharacterDao) {
...
class WholePeriodCharacterPaymentModel(private val paymentDao: PaymentDao, private val characterDao: RecommendCharacterDao) {
